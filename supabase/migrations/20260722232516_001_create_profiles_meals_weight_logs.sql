...
Users sign in via Google OAuth (Supabase Auth). All data is scoped to the
...
- `email_encrypted` (text) — user email encrypted at rest with `pgp_sym_encrypt`.
  The plaintext email is never stored in this table; Supabase Auth already
  stores it in `auth.users` (managed by Supabase). This column provides an
  additional at-rest encrypted copy for application-level lookups.
- `email_hash` (text) — SHA-256 hash of the email for dedup/lookup without
  decrypting. Indexed.
- `display_name` (text) — user-chosen display name.
- `avatar_url` (text) — profile picture URL (e.g. from Google OAuth).
...
- `profiles`: users can SELECT/INSERT/UPDATE/DELETE only their own row
  (`auth.uid() = id`).
- `meals`: users can CRUD only their own rows (`auth.uid() = profile_id`).
- `weight_logs`: users can CRUD only their own rows (`auth.uid() = profile_id`).

## 3. Encryption
- The `pgcrypto` extension is enabled for `pgp_sym_encrypt`/`pgp_sym_decrypt`.
- `email_encrypted` stores the email encrypted with a symmetric key derived
  from the project's database URL secret. This protects emails at rest.
- `email_hash` stores a SHA-256 digest for lookups without decryption.
- All client-server communication is encrypted in transit via TLS (HTTPS),
  which Supabase enforces by default.

## 4. Indexes
- `idx_profiles_email_hash` on `profiles(email_hash)` for email lookups.
- `idx_meals_profile_logged` on `meals(profile_id, logged_at DESC)` for
  efficient date-range queries.
- `idx_weight_logs_profile_logged` on `weight_logs(profile_id, logged_at DESC)`.

## 5. Important Notes
1. The `profile_id` columns on `meals` and `weight_logs` default to
   `auth.uid()` so frontend inserts that omit the owner still satisfy RLS.
2. The `profiles.id` column references `auth.users(id)` with `ON DELETE CASCADE`
   so profile data is cleaned up if a user is deleted from auth.
3. `pgcrypto` is enabled in the `extensions` schema (Supabase convention).
*/
...
-- Encrypts the user's email at rest using pgp_sym_encrypt.
-- ============================================================
